...
function getCorrectAction(hand) {
    ...
    // 1. Check Deviations (Illustrious 18)
    if (countingEnabled) {
        if (total === 16 && upVal === 10 && tc >= 0) return 'stand';
        if (total === 15 && upVal === 10 && tc >= 4) return 'stand';
        if (total === 12 && upVal === 3 && tc >= 2) return 'stand';
        if (total === 12 && upVal === 2 && tc >= 3) return 'stand';
    }
    ...
    // Default to Hit for this simplified example (can be expanded with full tables)
    return 'hit';
}
...
function playerMove(action) {
    ...
    // Check if the move was correct BEFORE executing it
    if (!moveJustMade) {
        totalDecisions++;
...
function checkCount() {
    ...
    if (guess === actual) {
        document.getElementById('count-feedback').innerText = "✓ Correct Count!";
