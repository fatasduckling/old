// script.js - FINAL FIXED: Deal button returns + Running count fully works
...
function updateDisplay() {
    ...
    let dealerStr = (gamePhase === 'playing' || gamePhase === 'insurance' || gamePhase === 'dealer' || gamePhase === 'roundEnd')
        ? (dealerHand.length > 0 ? cardText(dealerHand[0]) + ' ??' : '')
...
function startHand() {
    ...
    // Hide end-round in case it was visible
...
function nextHand() {
    if (currentHandIndex < playerHands.length - 1) {
        ...
        if (allBust) {
            ...
            evaluateResults();  // Goes to count check even on bust
        } else {
            dealerPlay();
        }
    }
...
function dealerPlay() {
    ...
    while (calculateTotal(dealerHand) < 17 ||
        ...
        dealCard(dealerHand);
        updateDisplay();  // Optional: show dealer hitting live
    }
...
function evaluateResults() {
    ...
    // *** KEY FIX: Show the running count guess section ***
    ...
    updateDisplay();  // Keeps dealer cards visible, hides actions, etc.
}
...
function checkCount() {
    ...
    // *** KEY FIX: Reset for new hand after delay ***
    setTimeout(() => {
        // Clear everything
        document.getElementById('end-round').style.display = 'none';
        ...
        playerHands = [];
        dealerHand = [];
        seenCards = [];  // Optional: reset count each shoe? Or keep for multi-hand. Here we reset per hand for practice.

        gamePhase = 'betting';
        updateDisplay();  // *** This makes the Deal button appear ***
        updateBetSuggestion();
    }, 4000);
}
